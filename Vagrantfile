...
Vagrant.configure(2) do |config|
  ...
  # Provision
  config.vm.provision "os:swap", type: "shell", privileged: false, inline: <<-SHELL
...
EOT
  ...
  config.vm.provision "os:hostname", type: "shell", run: 'always', inline: <<-SHELL
  ...
  config.vm.provision "nginx", type: "shell", privileged: false, inline: <<-SHELL
    sudo apt-get install -y nginx
  SHELL

  config.vm.provision "nginx:no-default", type: "shell", privileged: false, inline: <<-SHELL
  ...
  config.vm.provision "angular:ng", type: "shell", privileged: false, inline: <<-SHELL
    sudo npm install -g @angular/cli
    sudo ng analytics off
  SHELL

  config.vm.provision "angular:ng-proxy", type: "shell", privileged: false, inline: <<-SHELL
    sudo tee /etc/nginx/sites-available/ng.serve > /dev/null <<"EOT"
      server {
...
EOT
    sudo ln -s /etc/nginx/sites-available/ng.serve /etc/nginx/sites-enabled/
    sudo systemctl restart nginx
  SHELL

  config.vm.provision "browser:chromium", type: "shell", privileged: false, inline: <<-SHELL
